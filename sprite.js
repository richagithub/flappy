...
function initSprites(img) {
	...
	s_numberS.draw = s_numberB.draw = function(ctx, x, y, num,center,padding) {
		num = num.toString();//just to get length of score,and chop each digit , we convert it into string arr.
		var step = this.width + 2;//width of each number in s_numberB/s_numberS
		if(center){
			x=center-(num.length*step -2)/2;//////???
		}
		if(padding){
			x+=step*(padding-num.length);//(padding-num.length) is 10-no. of digits in score, we are taking 10 as the max score length limit. 
		}
